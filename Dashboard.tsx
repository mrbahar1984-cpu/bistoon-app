...
import { Clock, Play, Square, Coffee, LogOut, UserPlus, LogIn, Fingerprint, RefreshCcw } from 'lucide-react';
...
const Dashboard: React.FC<Props> = ({ currentUser, onLogin }) => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (isRegister) {
      ...
      if (error) alert('کد ملی یا رمز عبور اشتباه است');
      else {
        // دریافت لاگ‌های قبلی
  ...
  const addLog = async (type: LogType) => {
    ...
    if (error) {
      alert('خطا در همگام‌سازی با سرور. لطفاً اینترنت خود را چک کنید.');
    } else {
    ...
    return (
      <div className="max-w-md mx-auto mt-12">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-white/50 backdrop-blur-sm">
          ...
          <p className="text-center text-slate-400 text-sm mb-8">سامانه ابری بیستون</p>
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <button 
              disabled={syncing}
              className="w-full bg-indigo-600 text-white p-4 rounded-2xl font-bold shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2"
            >
              {syncing ? <RefreshCcw className="animate-spin" size={20} /> : (isRegister ? <UserPlus size={20} /> : <LogIn size={20} />)}
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="bg-emerald-50 text-emerald-700 p-4 rounded-2xl text-xs font-bold flex items-center justify-between border border-emerald-100">
        <span>وضعیت: متصل به دیتابیس ابری (آنلاین)</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex items-center justify-between">
            ...
            <div className="bg-slate-100 px-4 py-2 rounded-xl text-[10px] font-bold text-slate-500">
        ...
        <div className="space-y-6">
           <div className="bg-indigo-600 text-white p-6 rounded-[2rem] shadow-xl">
              ...
              <p className="text-xs text-indigo-100 leading-relaxed">
                این لیست به صورت زنده از دیتابیس ابری خوانده می‌شود.
              </p>
