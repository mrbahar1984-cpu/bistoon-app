...
import { Play, Square, Coffee, LogOut, UserPlus, LogIn, Fingerprint, RefreshCcw, Wifi, AlertCircle } from 'lucide-react';
...
const Dashboard: React.FC<Props> = ({ currentUser, onLogin }) => {
      ...
      if (isRegister) {
        ...
        onLogin({ 
          ...
          logs: logs?.map(l => ({ 
            ...
            shamsiDate: l.shamsi_date || l.shams_date, // پشتیبانی از هر دو حالت برای احتیاط
            time: l.time 
          })) || [] 
        });
      }
  ...
  const addLog = async (type: LogType) => {
    ...
    try {
      // توجه: نام ستون در اینجا shamsi_date قرار داده شد
      ...
      alert(`❌ خطای ثبت ابری: ${err.message}\n(احتمالا نام ستون در دیتابیس متفاوت است)`);
    }
    setSyncing(false);
  };
      ...
      <div className="max-w-md mx-auto mt-12 px-4">
        <div className="bg-[#1e293b] p-10 rounded-[3.5rem] shadow-2xl border border-slate-700">
          ...
          <h2 className="text-2xl font-black text-center text-white mb-8">
            {isRegister ? 'عضویت در سامانه' : 'ورود به بیستون 5.3'}
          </h2>
    ...
    <div className="max-w-4xl mx-auto space-y-8 p-4">
      <div className="bg-indigo-500/10 text-indigo-300 p-4 rounded-[2rem] text-[10px] font-black flex items-center justify-between border border-indigo-500/20">
        <div className="flex items-center gap-3">
           ...
           <span>نسخه ۵.۳ | پایداری ستون‌ها</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          <div className="bg-[#1e293b] p-8 rounded-[2.5rem] border border-slate-700 flex items-center justify-between shadow-xl">
             <div className="flex items-center gap-6">
                <div className="bg-indigo-600 p-5 rounded-2xl text-white font-black text-3xl shadow-lg">{getShamsiTime()}</div>
                <div>
          ...
          <div className="bg-[#1e293b] rounded-[2.5rem] border border-slate-700 overflow-hidden">
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-8 rounded-[3rem] shadow-xl text-white text-center h-fit">
           <h3 className="text-xl font-black">{currentUser.name}</h3>
           <p className="text-[10px] mt-2 opacity-60">کد ملی: {currentUser.nationalId}</p>
           <div className="mt-12 bg-black/20 p-8 rounded-[2.5rem]">
              ...
              <p className="text-[10px] mt-4 font-bold uppercase tracking-widest opacity-70">ثبت‌های امروز</p>
           </div>
        </div>
      </div>
    </div>
