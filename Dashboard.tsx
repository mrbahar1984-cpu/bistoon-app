...
const Dashboard: React.FC<Props> = ({ currentUser, onLogin }) => {
  ...
  const submitReq = async (e: React.FormEvent) => {
    ...
    if (reqForm.type === 'REMOTE_WORK') {
        // محاسبه اعشاری: اگر دقیقه غیر صفر باشد عدد اعشاری تولید می شود
    ...
    if (!error) {
        ...
        alert(`خطا در ثبت درخواست: احتمالاً ستون amount در دیتابیس شما از نوع Integer است و عدد اعشاری قبول نمی‌کند. لطفاً در Supabase نوع ستون amount را به numeric یا float تغییر دهید.`);
    }
    setSyncing(false);
  };
