...
const Dashboard: React.FC<Props> = ({ currentUser, onLogin }) => {
  ...
  const submitReq = async (e: React.FormEvent) => {
    ...
    const inlineKeyboard = [
      [
        { text: "✅ تایید", callback_data: `approve:${requestId}` },
        { text: "❌ رد", callback_data: `reject:${requestId}` }
      ]
    ];
    ...
    try {
      ...
      if (hasTelegramError) {
        feedback += ' (خطای ارسال پیام تلگرام)';
      } else {
      ...
      setSuccessMsg('درخواست ثبت شد اما در ارسال پیام تلگرام خطایی رخ داد.');
    }
