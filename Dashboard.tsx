...
const Dashboard: React.FC<Props> = ({ currentUser, onLogin }) => {
  ...
  const submitReq = async (e: React.FormEvent) => {
    e.preventDefault();
    setSyncing(true);
    let amount = (reqForm.type === 'REMOTE_WORK') ? Number((reqForm.h + (reqForm.m / 60)).toFixed(2)) : (reqForm.type === 'DAILY_LEAVE' ? reqForm.days : 1);
    ...
    if (!error && data) {
      setSuccessMsg('درخواست شما ثبت شد.');
          ...
          <div className="md:col-span-4 space-y-6">
            <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
              ...
              <form onSubmit={submitReq} className="space-y-4">
                <select className="w-full p-4 bg-slate-50 rounded-2xl font-black text-xs outline-none" value={reqForm.type} onChange={e => setReqForm({...reqForm, type: e.target.value as LeaveType})}>
                ...
                <input className="w-full p-4 bg-slate-50 rounded-2xl font-mono font-bold text-center outline-none" value={reqForm.date} onChange={e => setReqForm({...reqForm, date: e.target.value})} />
                <textarea className="w-full p-4 bg-slate-50 rounded-2xl font-bold text-xs outline-none" rows={3} placeholder="توضیحات..." value={reqForm.desc} onChange={e => setReqForm({...reqForm, desc: e.target.value})} />
      ...
      {activeTab === 'REQUEST_STATUS' && (
        <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {myRequests.map((r, i) => (
              <div key={i} className="p-6 bg-slate-50 rounded-[2.5rem] border relative overflow-hidden group hover:bg-white transition-all">
...
const TabBtn = React.memo(({ active, label, icon, onClick }: any) => (
...
const ActionBtn = React.memo(({ icon, label, color, onClick }: any) => (
...
const ColleagueItem = React.memo(({ colleague }: { colleague: any }) => (
