...
import { getShamsiDate, getShamsiTime, getDayName, isHoliday } from './jalali';
...
const Dashboard: React.FC<Props> = ({ employee, onSave }) => {
    ...
    return (
      <div className="max-w-md mx-auto bg-white p-8 rounded-3xl shadow-xl mt-10 text-center">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <input required className="w-full p-4 border rounded-2xl bg-slate-50" placeholder="نام و نام خانوادگی" value={name} onChange={e => setName(e.target.value)} />
          <input required className="w-full p-4 border rounded-2xl bg-slate-50" placeholder="کد پرسنلی" value={employeeId} onChange={e => setEmployeeId(e.target.value)} />
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <button onClick={copyDailyToken} className="w-full bg-indigo-600 text-white p-4 rounded-2xl font-bold flex items-center justify-center gap-2">
      ...
      <div className="bg-white rounded-3xl shadow-sm border overflow-hidden">
        ...
        <div className="max-h-60 overflow-y-auto">
          {employee.logs.map(log => (
            <div key={log.id} className="p-4 border-b flex justify-between text-sm">
