...
const Dashboard: React.FC<Props> = ({ currentUser, onLogin }) => {
  ...
  const submitReq = async (e: React.FormEvent) => {
    e.preventDefault();
    setSyncing(true);
    // افزودن فیلد amount جهت اطمینان از ثبت صحیح در دیتابیس و نمایش
    const amount = reqForm.h + (reqForm.m / 60);
      ...
      {activeTab === 'MAIN' && (
        <div className="grid md:grid-cols-12 gap-6 animate-in fade-in">
          ...
          <div className="md:col-span-4 space-y-6">
            <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
              <h3 className="text-lg font-black mb-6 flex items-center gap-2 text-indigo-600 border-b pb-4"><Send size={20}/> ثبت درخواست دورکاری</h3>
              <form onSubmit={submitReq} className="space-y-4">
                ...
                <button disabled={syncing} className="w-full bg-indigo-600 text-white p-4 rounded-2xl font-black text-sm shadow-lg hover:bg-indigo-700 transition-all">ارسال درخواست دورکاری</button>
              </form>
            </div>
          </div>
        </div>
            ...
            {myRequests.map((r, i) => (
              <div key={i} className="p-6 bg-slate-50 rounded-[2.5rem] border hover:shadow-md transition-all">
                ...
                <div className="text-[11px] font-black text-slate-500 bg-white p-3 rounded-xl border border-slate-100 flex justify-between items-center">
                  <span>مدت زمان:</span>
                  <span className="font-mono text-indigo-600">{r.remote_hours} ساعت و {r.remote_minutes} دقیقه</span>
                </div>
              </div>
            ))}
