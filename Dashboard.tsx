...
const Dashboard: React.FC<Props> = ({ currentUser, onLogin }) => {
  ...
  const addLog = async (type: LogType) => {
    ...
    await supabase.from('attendance_logs').insert([{
      ...
      type,
      shamsi_date: toEnglishDigits(getShamsiDate()),
      time: getShamsiTime(),
      timestamp: Date.now()
    }]);
    loadData();
        ...
        <div className="grid md:grid-cols-12 gap-6 animate-in fade-in">
          ...
          <div className="md:col-span-4 space-y-6">
            <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
              ...
              {successMsg && (
                <div className="mb-4 p-4 bg-emerald-50 text-emerald-700 rounded-2xl text-xs font-bold animate-pulse border border-emerald-100">
                  {successMsg}
                </div>
              )}
              ...
              <form onSubmit={submitReq} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <select required className="w-full p-3 bg-slate-50 rounded-xl border-none outline-none font-bold" value={reqForm.type} onChange={e => setReqForm({...reqForm, type: e.target.value as LeaveType})}>
                ...
                <button disabled={syncing} className="w-full bg-indigo-600 text-white p-4 rounded-2xl font-black text-sm shadow-lg hover:bg-indigo-700 transition-all">
                  {syncing ? <RefreshCcw className="animate-spin inline-block mr-2" /> : null}
                  ارسال درخواست
                </button>
              </form>
            </div>
