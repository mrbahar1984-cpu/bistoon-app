...
const Dashboard: React.FC<Props> = ({ employee, onSave }) => {
  ...
  const downloadReport = () => {
    ...
    const data = JSON.stringify({ n: employee.name, i: employee.employeeId, d: today, l: todayLogs }, null, 2);
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Report_${employee.employeeId}_${today.replace(/\//g, '-')}.json`;
  ...
  const copyDailyToken = (e: React.MouseEvent) => {
    ...
    const jsonStr = JSON.stringify({ n: employee.name, i: employee.employeeId, d: today, l: todayLogs });
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">کد پرسنلی: {employee.employeeId}</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <ActionButton 
          ...
          label="پاس ساعتی" 
          color="bg-amber-500" 
          onClick={() => addLog(LogType.HOURLY_PASS_START)}
        />
        <ActionButton 
          icon={<LogOut size={28} />} 
          label="پایان مرخصی" 
          color="bg-slate-600" 
          onClick={() => addLog(LogType.HOURLY_PASS_END)}
        />
      </div>
...
const getLogColor = (type: LogType) => {
  switch (type) {
    ...
    case LogType.HOURLY_PASS_START: return 'bg-amber-50 text-amber-600 border border-amber-100';
    case LogType.HOURLY_PASS_END: return 'bg-slate-100 text-slate-600 border border-slate-200';
