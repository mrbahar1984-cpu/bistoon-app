...
const Dashboard: React.FC<Props> = ({ currentUser, onLogin }) => {
  ...
  const submitReq = async (e: React.FormEvent) => {
    ...
    if (reqForm.type === 'REMOTE_WORK') {
      ...
      amount = 1;
      description = `[پاس ${reqForm.passType}] ${reqForm.desc}`;
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        else alert('خطا در ورود: کد ملی یا رمز عبور اشتباه است.');
      }
  ...
  const handleSetActiveTabMain = () => setActiveTab('MAIN');
  const handleSetActiveTabHistory = () => setActiveTab('FULL_HISTORY');
  const handleSetActiveTabRequests = () => setActiveTab('REQUEST_STATUS');
...
const ActionBtn = React.memo(({ icon, label, color, onClick }: any) => (
  <button onClick={onClick} className={`${color} text-white p-6 rounded-[2.5rem] flex flex-col items-center gap-3 shadow-lg hover:scale-105 active:scale-95 transition-all w-full`}>
