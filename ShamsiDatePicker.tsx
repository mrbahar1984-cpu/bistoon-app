...
// توابع تقویمی خودکفا برای اطمینان از عملکرد مستقل تقویم حتی در صورت عدم بروزرسانی jalali.ts
...
const getFirstDayOfWeekInShamsiMonth = (year: number, month: number): number => {
...
const formatShamsiLong = (shamsiStr: string): string => {
  ...
  const gDate = new Date(g.gy, g.gm - 1, g.gd);
  const dayName = new Intl.DateTimeFormat('fa-IR', { weekday: 'long' }).format(gDate);
...
export interface ShamsiDatePickerProps {
  ...
  minYear?: number;
  maxYear?: number;
  theme?: 'emerald' | 'indigo' | 'slate' | 'blue' | 'teal' | 'rose' | 'amber';
...
export const ShamsiDatePicker: React.FC<ShamsiDatePickerProps> = ({
  ...
  const themeColors = {
    ...
    teal: {
      activeDay: 'bg-teal-600 text-white shadow-md shadow-teal-600/30',
      todayRing: 'ring-2 ring-teal-500 text-teal-700 font-black',
      headerBg: 'bg-teal-50 text-teal-950 border-teal-100',
      iconBtn: 'hover:bg-teal-100 text-teal-800',
      focusBorder: 'focus:ring-2 focus:ring-teal-500 focus:border-teal-500',
      badge: 'bg-teal-50 text-teal-700 border-teal-200',
      accentBtn: 'bg-teal-600 hover:bg-teal-700 text-white'
    },
    rose: {
      activeDay: 'bg-rose-600 text-white shadow-md shadow-rose-600/30',
      todayRing: 'ring-2 ring-rose-500 text-rose-700 font-black',
      headerBg: 'bg-rose-50 text-rose-950 border-rose-100',
      iconBtn: 'hover:bg-rose-100 text-rose-800',
      focusBorder: 'focus:ring-2 focus:ring-rose-500 focus:border-rose-500',
      badge: 'bg-rose-50 text-rose-700 border-rose-200',
      accentBtn: 'bg-rose-600 hover:bg-rose-700 text-white'
    },
    amber: {
      activeDay: 'bg-amber-600 text-white shadow-md shadow-amber-600/30',
      todayRing: 'ring-2 ring-amber-500 text-amber-700 font-black',
      headerBg: 'bg-amber-50 text-amber-950 border-amber-100',
      iconBtn: 'hover:bg-amber-100 text-amber-800',
      focusBorder: 'focus:ring-2 focus:ring-amber-500 focus:border-amber-500',
