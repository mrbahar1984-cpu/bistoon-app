...
import { ShieldAlert, Users, FileText, Settings, Check, X, Trash2, Printer, Download, Plus, Calendar, Monitor, Menu, Briefcase, RefreshCcw, LayoutGrid, ClipboardCheck, Clock } from 'lucide-react';
...
const AdminPanel: React.FC = () => {
  ...
  const [activeMenu, setActiveMenu] = useState<'REPORTS' | 'REQUESTS' | 'USERS' | 'SETTINGS'>('REPORTS');
  ...
  const fetchBaseData = async () => {
    ...
    if (emps) {
      for (const emp of emps) {
        // فیلتر کردن لاگ‌ها بر اساس بازه انتخابی در گزارش‌گیری
  ...
  useEffect(() => { if (adminAuth) fetchBaseData(); }, [adminAuth, dutyConfig.month, dateRange.from, dateRange.to]);
    ...
    <div className="flex flex-col lg:flex-row min-h-screen -m-8">
        ...
        {sidebarOpen && (
          <div className="mt-auto bg-slate-50 p-4 rounded-3xl border border-slate-100">
             <div className="flex items-center gap-3">
                ...
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Active Admin Session</span>
             </div>
          </div>
        )}
      </aside>
      ...
      <main className="flex-1 p-4 lg:p-10 overflow-y-auto">
        <header className="no-print flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
           <div>
              <h2 className="text-2xl font-black text-slate-800">{activeMenu === 'REPORTS' ? 'گزارش‌گیری و محاسبات' : activeMenu === 'REQUESTS' ? 'مدیریت درخواست‌ها' : activeMenu === 'SETTINGS' ? 'پیکربندی سامانه' : 'پرسنل'}</h2>
           ...
           <div className="flex gap-2">
              ...
              <button onClick={() => window.print()} className="flex items-center gap-2 bg-slate-800 text-white px-5 py-3 rounded-2xl font-black text-xs shadow-lg"><Printer size={16}/> چاپ گزارش</button>
