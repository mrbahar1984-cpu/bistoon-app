...
const AdminPanel: React.FC = () => {
  ...
  const generateDynamoReport = async () => {
    ...
    Object.keys(grouped).forEach(key => {
      ...
      const remote = approvedReqs?.filter(r => r.employee_id === empId && r.shamsi_date === date && r.type === 'REMOTE_WORK')
        .reduce((sum, r) => sum + (r.remote_hours || 0) + (r.remote_minutes || 0) / 60, 0) || 0;
    ...
    setMaxClocks(tempMaxClocks);
    setMaxPasses(tempMaxPasses);
  ...
  const handleBackupAndCleanup = async () => {
    ...
    if (!confirm(`هشدار: تمامی ترددهای ثبت شده تا تاریخ ${targetDate} پشتیبان‌گیری و سپس حذف خواهند شد. ادامه می‌دهید؟`)) return;

    setLoading(true);
    const { data: logsToBackup } = await supabase.from('attendance_logs').select('*').lte('shamsi_date', targetDate);
    ...
    if (logsToBackup && logsToBackup.length > 0) {
        ...
        const link = document.createElement("a");
        link.href = URL.createObjectURL(blob);
        link.download = `BaharTime_Backup_Until_${targetDate}.csv`;
        link.click();

        // حذف از دیتابیس
        const { error } = await supabase.from('attendance_logs').delete().lte('shamsi_date', targetDate);
        if (!error) alert(`پشتیبان‌گیری انجام شد و ${logsToBackup.length} مورد تردد با موفقیت حذف گردید.`);
        else alert('خطا در حذف داده‌ها');
    } else {
        alert('ترددی در این بازه زمانی برای حذف یافت نشد.');
    }
    setLoading(false);
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen -m-8">
      ...
      <main className="flex-1 p-6 lg:p-10 bg-slate-50/50 overflow-x-hidden">
        ...
        {activeMenu === 'REQUESTS' && (
          <div className="bg-white p-10 rounded-[3rem] shadow-sm border animate-in fade-in">
            <h2 className="text-2xl font-black mb-10 text-slate-800 border-b pb-6">تایید درخواست‌های تردد/دورکاری</h2>
            <div className="grid md:grid-cols-2 gap-8">
              {requests.filter(r => r.status === 'PENDING').map(r => (
                <div key={r.id} className="p-8 bg-slate-50 rounded-[3rem] border border-slate-100 hover:shadow-lg transition-all">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <span className="bg-indigo-600 text-white px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter shadow-md shadow-indigo-100">{r.type.replace(/_/g, ' ')}</span>
                  </div>
                  <div className="bg-white p-4 rounded-2xl mb-8">
                    ...
                    <p className="text-[10px] font-black text-indigo-600 mt-3 border-t pt-3">
                        {r.type === 'REMOTE_WORK' ? `مقدار درخواستی: ${r.remote_hours} ساعت و ${r.remote_minutes} دقیقه` :
                         r.type === 'DAILY_LEAVE' ? `تعداد روز درخواستی: ${r.amount} روز` : 'پاس ساعتی'}
                    </p>
                  </div>
                  <div className="flex gap-3">
                    <button onClick={() => updateRequestStatus(r.id, 'APPROVED')} className="flex-1 bg-emerald-500 text-white py-3 rounded-2xl text-xs font-black shadow-lg shadow-emerald-100 hover:bg-emerald-600 active:scale-95 transition-all">تایید نهایی</button>
                    <button onClick={() => updateRequestStatus(r.id, 'REJECTED')} className="flex-1 bg-rose-500 text-white py-3 rounded-2xl text-xs font-black shadow-lg shadow-rose-100 hover:bg-rose-600 active:scale-95 transition-all">رد درخواست</button>
                  </div>
                </div>
              ))}
              {requests.filter(r => r.status === 'PENDING').length === 0 && <p className="text-center text-slate-300 py-32 italic font-bold text-lg col-span-full">هیچ درخواستی در صف انتظار نیست.</p>}
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="bg-white p-10 rounded-[3rem] shadow-sm border flex flex-wrap gap-6 items-end no-print">
              <div className="w-full md:w-[200px] space-y-2">
                ...
                <select className="w-full p-4 bg-slate-50 rounded-2xl border font-bold outline-none focus:ring-2 focus:ring-emerald-500" value={reportFilterEmpId} onChange={e => setReportFilterEmpId(e.target.value)}>
              ...
              <div className="flex-1 min-w-[150px] space-y-2">
                ...
                <input className="w-full p-4 bg-slate-50 rounded-2xl border font-mono font-bold text-center outline-none focus:ring-2 focus:ring-emerald-500" value={reportRange.from} onChange={e => setReportRange({...reportRange, from: e.target.value})} />
              </div>
              <div className="flex-1 min-w-[150px] space-y-2">
                <label className="text-[11px] font-black text-slate-400 mr-2 tracking-widest uppercase">تا تاریخ</label>
                <input className="w-full p-4 bg-slate-50 rounded-2xl border font-mono font-bold text-center outline-none focus:ring-2 focus:ring-emerald-500" value={reportRange.to} onChange={e => setReportRange({...reportRange, to: e.target.value})} />
              </div>
            ...
            <div className="max-w-xl space-y-8">
                <div className="bg-amber-50 p-6 rounded-[2rem] border border-amber-200">
                    <p className="text-sm font-bold text-amber-800 leading-relaxed mb-4">
                        برای حفظ سرعت برنامه، می‌توانید ترددهای قدیمی را حذف کنید. قبل از حذف، یک فایل Excel پشتیبان بصورت خودکار دریافت خواهید کرد.
                    </p>
                    <div className="space-y-4">
                        <div>
                            <label className="text-xs font-black text-amber-700 block mb-2">حذف تمامی ترددها تا تاریخ (و خودِ تاریخ):</label>
                            <input className="w-full p-4 bg-white rounded-2xl border border-amber-300 font-mono font-bold text-center outline-none" value={cleanupDate} onChange={e => setCleanupDate(e.target.value)} />
                        </div>
                        <button onClick={handleBackupAndCleanup} disabled={loading} className="w-full bg-rose-600 text-white p-5 rounded-2xl font-black flex items-center justify-center gap-3 shadow-xl shadow-rose-100 hover:bg-rose-700 transition-all">
                            {loading ? <RefreshCcw className="animate-spin" /> : <Eraser size={20}/>}
                            پشتیبان‌گیری اکسل و حذف قطعی داده‌ها
                        </button>
                    </div>
                </div>
            </div>
