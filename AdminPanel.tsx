...
import { 
  ...
  ChevronRight, ChevronLeft, Download, Edit2, Plus, Save, Clock
} from 'lucide-react';
...
const AdminPanel: React.FC = () => {
  ...
  const [reportFilterEmpId, setReportFilterEmpId] = useState<string>(''); // فیلتر انتخاب کارمند در گزارش
  ...
  const generateDynamoReport = async () => {
    ...
    // کوئری داینامیک برای فیلتر بازه زمانی و کارمند (در صورت انتخاب)
  ...
  const exportExcel = () => {
    ...
    dynamoData.forEach((row, idx) => {
      ...
      // Clocks
      for(let i=0; i < maxClocks * 2; i++) {
        csv += `,${row.clocks[i] ? row.clocks[i].time : ''}`;
      }
      // Passes
      for(let i=0; i < maxPasses * 2; i++) {
        csv += `,${row.passes[i] ? row.passes[i].time : ''}`;
      }
      csv += `,${row.remoteHours}\n`;
    });
      ...
      <main className="flex-1 p-6 lg:p-10 bg-slate-50/50 overflow-x-hidden">
              ...
              {requests.filter(r => r.status === 'PENDING').map(r => (
                <div key={r.id} className="p-8 bg-slate-50 rounded-[3rem] border border-slate-100 hover:shadow-lg transition-all">
                  ...
                  <div className="bg-white p-4 rounded-2xl mb-8">
                    ...
                    <p className="text-[10px] font-black text-indigo-600 mt-3 border-t pt-3">مقدار درخواستی: {r.remote_hours} ساعت و {r.remote_minutes} دقیقه</p>
                  </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-right text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      ...
                      {empLogs.length === 0 && <tr><td colSpan={4} className="p-20 text-center text-slate-300 italic font-bold text-lg">ترددی برای این کاربر یافت نشد.</td></tr>}
                    </tbody>
                  </table>
                </div>
              </div>
        ...
        {activeMenu === 'DYNAMO_REPORTS' && (
          <div className="space-y-6">
            ...
            {dynamoData.length > 0 && (
              <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border overflow-x-auto printable-area">
                <div className="hidden print:block text-center mb-10 border-b-2 border-slate-800 pb-6">
                  <h1 className="text-3xl font-black">گزارش تفصیلی ترددهای پرسنل</h1>
                  <p className="text-sm font-bold mt-2">بازه زمانی: {reportRange.from} تا {reportRange.to}</p>
                </div>
            ...
            {dynamoData.length === 0 && !loading && <div className="text-center py-40 text-slate-300 italic font-bold text-xl">بازه زمانی مورد نظر را انتخاب و دکمه مشاهده گزارش را کلیک کنید.</div>}
          </div>
        )}
      </main>
