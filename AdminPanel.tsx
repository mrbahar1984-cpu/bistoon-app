...
import { ShieldAlert, Users, FileText, Check, Trash2, Printer, Download, Calendar, Menu, RefreshCcw, Activity, Search, FileSpreadsheet, Eye, X } from 'lucide-react';
...
const AdminPanel: React.FC = () => {
  ...
  const generateMonthlyReport = async () => {
    setLoading(true);
    // ساخت الگوی جستجو برای ماه (مثلا 1403/02/%)
    const monthPattern = `${toEnglishDigits(selectedYear)}/${toEnglishDigits(selectedMonth)}/%`;
              ...
              <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
                <table className="w-full text-right text-xs">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="p-5 rounded-r-[1.5rem] font-black">کارمند</th>
                      <th className="p-5 font-black">تاریخ</th>
                      <th className="p-5 font-black">ساعت</th>
                      <th className="p-5 rounded-l-[1.5rem] font-black">نوع</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {reportData.map((row, i) => (
                      <tr key={i} className="hover:bg-slate-50/50">
                        ...
                        <td className="p-5"><span className="px-3 py-1 rounded-full bg-slate-100 text-[9px] font-black">{row.type}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="space-y-6">
            <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-6 items-end no-print">
               ...
               <button onClick={generateMonthlyReport} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black text-sm shadow-lg hover:bg-indigo-700 transition-all flex items-center gap-2">
                  {loading ? <RefreshCcw className="animate-spin" size={18}/> : <Eye size={18}/>} نمایش کارکرد
               </button>
            </div>
            {reportData.length > 0 && (
              <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 printable-area">
                <div className="hidden print:block text-center mb-10 border-b pb-6">
                   <h1 className="text-2xl font-black">گزارش سرجمع کارکرد ماهانه</h1>
                   <p className="text-sm font-bold mt-2">{shamsiMonthNames[parseInt(selectedMonth)-1]} {selectedYear}</p>
                </div>
                <table className="w-full text-right text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="p-5 rounded-l-[1.5rem] font-black">کارکرد خالص</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {reportData.map((row, i) => (
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               {requests.filter(r => r.status === 'PENDING').map((r, i) => (
                 <div key={i} className="p-6 bg-slate-50 rounded-[2.5rem] border">
        ...
        {activeMenu === 'USERS' && (
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {employees.map((e, i) => (
                  <div key={i} className="p-5 bg-slate-50 rounded-[2rem] border flex justify-between items-center">
                    <div className="flex items-center gap-3">
                    ...
                    <button onClick={async () => { if(confirm(`حذف ${e.name}؟`)) { await supabase.from('employees').delete().eq('id', e.id); fetchInitialData(); } }} className="text-slate-300 hover:text-rose-500"><Trash2 size={16}/></button>
                  </div>
                ))}
             </div>
          </div>
...
const ShamsiDatePicker = ({ value, onChange }: { value: string, onChange: (v: string) => void }) => {
  ...
  return (
    <div className="relative" ref={containerRef}>
      <div onClick={() => setOpen(!open)} className="w-full p-4 bg-slate-50 rounded-2xl text-sm font-mono font-bold outline-none border border-slate-100 cursor-pointer flex items-center justify-between">
      ...
      {open && (
        <div className="absolute top-full right-0 mt-2 bg-white p-4 rounded-3xl shadow-2xl border border-slate-100 z-[100] w-64">
          <div className="flex justify-between items-center mb-4">
             <button onClick={() => onChange(`${year}/${String(month === 1 ? 12 : month - 1).padStart(2, '0')}/${String(day).padStart(2, '0')}`)} className="p-1 hover:bg-slate-100 rounded-lg"><X size={14}/></button>
             <span className="font-black text-xs text-slate-800">{shamsiMonthNames[month-1]} {year}</span>
             <button onClick={() => setOpen(false)} className="text-rose-500 p-1"><X size={14}/></button>
          </div>
          <div className="grid grid-cols-7 gap-1">
            {days.map(d => (
              <button key={d} onClick={() => { onChange(`${year}/${String(month).padStart(2, '0')}/${String(d).padStart(2, '0')}`); setOpen(false); }} className={`p-2 text-[10px] font-bold rounded-xl ${d === day ? 'bg-emerald-600 text-white' : 'hover:bg-emerald-50 text-slate-600'}`}>
                {d}
              </button>
            ))}
          </div>
