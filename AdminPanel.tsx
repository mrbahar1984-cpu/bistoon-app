...
import { calculateWorkDetails } from './calculations'; // تغییر مسیر به ریشه
import { Users, RefreshCcw, Printer, Calculator, History, TrendingUp, Wifi, ShieldAlert } from 'lucide-react';
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      for (const emp of formatted) {
        ...
        if (logs) {
          const attendanceLogs: AttendanceLog[] = logs.map(l => ({ id: l.id, timestamp: l.timestamp, type: l.type as LogType, shamsiDate: l.shams_date, time: l.time }));
    ...
    return (
      <div className="max-w-md mx-auto mt-20 px-4">
        <div className="bg-[#1e293b] p-10 rounded-[3rem] shadow-2xl border border-slate-700 text-center">
          ...
          <h2 className="text-3xl font-black text-white mb-8">ورود ادمین 5.0</h2>
          <input type="password" placeholder="رمز" className="w-full p-6 rounded-3xl bg-slate-800 text-white text-center font-black mb-4" value={password} onChange={e => setPassword(e.target.value)} />
          <button onClick={() => password === 'admin123' ? setAdminAuth(true) : alert('غلط')} className="w-full bg-indigo-600 text-white p-6 rounded-3xl font-black">ورود</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 p-4">
      <header className="bg-[#1e293b] p-8 rounded-[3rem] border border-slate-700 flex justify-between items-center shadow-2xl">
        <h2 className="text-3xl font-black text-white">پنل مدیریت متمرکز</h2>
        <div className="flex gap-4">
          <button onClick={fetchAllData} className="bg-slate-700 text-white px-8 py-5 rounded-2xl flex items-center gap-3 font-black">
      ...
      <div className="grid md:grid-cols-4 gap-8">
        <aside className="bg-[#1e293b] p-8 rounded-[3rem] border border-slate-700 h-fit">
           <h3 className="font-black text-white mb-6 text-xl">پرسنل آنلاین</h3>
...
const StatCard = ({ label, value, icon }: any) => (
  <div className="bg-slate-800 p-6 rounded-3xl border border-slate-700">
    <div className="flex items-center gap-3 mb-3 text-slate-500 uppercase text-[10px] font-black">{icon} {label}</div>
