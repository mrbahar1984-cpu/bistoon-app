...
const AdminPanel: React.FC = () => {
  ...
  const editRequestAmount = async (id: string, currentAmount: number, type: string) => {
    ...
    if (newVal !== null && !isNaN(Number(newVal))) {
      const { error } = await supabase.from('leave_requests').update({ amount: Number(newVal) }).eq('id', id);
      if (error) alert("خطا در ویرایش: احتمالاً ستون مقدار در دیتابیس عدد اعشاری قبول نمی‌کند.");
      fetchData();
    }
  };
  ...
  const generateDynamoReport = async () => {
    ...
    // واکشی تمامی ترددها در بازه
    let logsQuery = supabase.from('attendance_logs').select('*').gte('shamsi_date', from).lte('shamsi_date', to);
    if (reportFilterEmpId) logsQuery = logsQuery.eq('employee_id', reportFilterEmpId);
    const { data: logs } = await logsQuery.order('timestamp', { ascending: true });
    
    // واکشی تمامی درخواست‌های تایید شده در بازه
    let reqsQuery = supabase.from('leave_requests').select('*').eq('status', 'APPROVED').gte('shamsi_date', from).lte('shamsi_date', to);
    if (reportFilterEmpId) reqsQuery = reqsQuery.eq('employee_id', reportFilterEmpId);
    ...
    // ۱. پردازش لاگ‌ها برای ایجاد ردیف‌های گزارش
    logs?.forEach(log => {
    ...
    // ۲. افزودن درخواست‌ها (حتی اگر لاگی وجود نداشته باشد)
    approvedReqs?.forEach(req => {
        const key = `${req.employee_id}_${req.shamsi_date}`;
        if (!grouped[key]) {
            const emp = employees.find(e => e.id === req.employee_id);
            grouped[key] = { 
              date: req.shamsi_date, 
              empName: emp?.name || 'ناشناس', 
              nid: emp?.nationalId || '---', 
              clocks: [], 
              passes: [],
        ...
        if (req.type === 'REMOTE_WORK') grouped[key].remoteHours += (req.amount || 0);
        if (req.type === 'DAILY_LEAVE') grouped[key].dailyLeave += (req.amount || 0);
    });

    // ۳. نهایی‌سازی ستون‌ها و محاسبات
    Object.keys(grouped).forEach(key => {
      const item = grouped[key];
      item.remoteHours = Number(item.remoteHours).toFixed(2);
    ...
    <div className="flex flex-col lg:flex-row min-h-screen -m-8">
      <aside className="w-full lg:w-64 bg-white border-l p-6 flex flex-col no-print">
        ...
        <nav className="space-y-3">
          <MenuBtn active={activeMenu === 'USERS'} label="مدیریت کاربران" icon={<Users size={20}/>} onClick={() => setActiveMenu('USERS')} />
      ...
      <main className="flex-1 p-6 lg:p-10 bg-slate-50/50 overflow-x-hidden">
        ...
        {activeMenu === 'REQUESTS' && (
          <div className="bg-white p-10 rounded-[3rem] shadow-sm border animate-in fade-in">
            <h2 className="text-2xl font-black mb-6 text-slate-800 border-b pb-6">تایید و مدیریت تمامی درخواست‌ها</h2>
            <div className="grid md:grid-cols-2 gap-8">
              {requests.map(r => (
                <div key={r.id} className={`p-8 rounded-[3rem] border border-slate-100 hover:shadow-lg transition-all ${r.status === 'PENDING' ? 'bg-slate-50 shadow-sm' : r.status === 'APPROVED' ? 'bg-emerald-50/30 border-emerald-100' : 'bg-rose-50/30 border-rose-100'}`}>
                  ...
                  <div className="bg-white p-4 rounded-2xl mb-8 border border-slate-50">
                    <p className="text-xs text-slate-500 italic leading-loose">"{r.description || 'بدون شرح مختصر'}"</p>
                    <p className="text-[11px] font-black text-indigo-600 mt-3 border-t pt-3">
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {r.status === 'PENDING' && (
                        <>
                            <button onClick={() => updateRequestStatus(r.id, 'APPROVED')} className="flex-1 bg-emerald-500 text-white py-3 rounded-2xl text-xs font-black hover:bg-emerald-600 active:scale-95 transition-all">تایید نهایی</button>
                            <button onClick={() => updateRequestStatus(r.id, 'REJECTED')} className="flex-1 bg-rose-500 text-white py-3 rounded-2xl text-xs font-black hover:bg-rose-600 active:scale-95 transition-all">رد درخواست</button>
                        </>
                    )}
                    {r.status === 'APPROVED' && (r.type === 'REMOTE_WORK' || r.type === 'DAILY_LEAVE') && (
                        <>
                            <button onClick={() => editRequestAmount(r.id, r.amount, r.type)} className="flex-1 bg-amber-500 text-white py-3 rounded-2xl text-xs font-black hover:bg-amber-600 transition-all flex items-center justify-center gap-2 shadow-sm"><Edit2 size={14}/> ویرایش مقدار</button>
                            <button onClick={() => deleteRequest(r.id)} className="flex-1 bg-slate-800 text-white py-3 rounded-2xl text-xs font-black hover:bg-slate-900 transition-all flex items-center justify-center gap-2 shadow-sm"><Trash2 size={14}/> حذف قطعی</button>
                        </>
                    )}
                    {(r.status === 'REJECTED' || (r.status === 'APPROVED' && r.type === 'HOURLY_PASS')) && (
                        <button onClick={() => deleteRequest(r.id)} className="flex-1 bg-slate-200 text-slate-600 py-3 rounded-2xl text-xs font-black hover:bg-rose-100 hover:text-rose-600 transition-all flex items-center justify-center gap-2"><Trash2 size={14}/> حذف از لیست</button>
                    )}
                  </div>
                </div>
              ))}
              {requests.length === 0 && <p className="text-center text-slate-300 py-32 italic font-bold text-lg col-span-full">هیچ درخواستی در سامانه ثبت نشده است.</p>}
            </div>
          </div>
        )}
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-sm border flex flex-col md:flex-row gap-6 items-center">
              <div className="flex-1 w-full">
                ...
                <select className="w-full p-4 bg-slate-50 rounded-2xl outline-none border focus:ring-2 focus:ring-emerald-500 transition-all font-bold" value={selectedEmp} onChange={e => fetchEmpLogs(e.target.value)}>
                  <option value="">-- یکی از کارکنان را انتخاب کنید --</option>
                  ...
                  <table className="w-full text-right text-sm">
                    <thead className="bg-slate-50">
                      <tr>
                        ...
                        <th className="p-5 font-black text-slate-400">ساعت</th>
                ...
                <table className="w-full text-right text-[10px] border-collapse min-w-[1200px]">
                  <thead className="bg-slate-50">
                    <tr className="border-b">
                      ...
                      <th className="p-4 font-black text-indigo-600 border-l">دورکاری (h)</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {dynamoData.map((row, i) => (
                      <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="p-4 font-mono font-black text-indigo-700 text-center text-base border-l">{row.remoteHours}</td>
